...
//      fprintf(stderr, "addLayer: the layer name " + currLayerName + " has already been used for a previous layer in this network\n");
    ...
    if (layerNames.find(inLayerName) == layerNames.end() && inLayerName != "") // first layer gets a free pass TODO should/can something be done so users don't have to use a blank string during the first layer
    {
